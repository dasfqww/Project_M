...
void UMConfirmScreenWidget::InitConfirmScreen(UConfirmScreenInfoObject* InScreenInfoObject, 
	TFunction<void(EConfirmScreenButtonType)> ClickedButtonCallback)
{
	check(InScreenInfoObject && TitleTextBlock && MessageTextBlock && ButtonsEntryBox);
	...
	if (ButtonsEntryBox->GetNumEntries() != 0)
	{
		...
		*/
		ButtonsEntryBox->GetAllEntries().Last()->SetFocus();
	}
}